...
Card.prototype.create = function() {
	...
	for (var i = 0; i < this.components.length; i++) {
        ...
        var components = this.components[i];

		if (components[5] && !this.phone) { //check if phone number is given
			//do not create phone component if not defined

		} else if (this.components[i][0] == 'el') { //create element and append to DOM
			var element = document.createElement('div');
			element.setAttribute('class', components[1]);
			element.setAttribute('id', components[2]);
			var location = document.getElementById(components[4]);
			location.appendChild(element);

		} else if (this.components[i][0] == 'txt') { //create text node and append to DOM
			var element = document.createTextNode(components[3]);
			var location = document.getElementById(components[4]);
			location.appendChild(element);
